	...
	gl2d::Texture bulletsTexture, gl2d::TextureAtlasPadding bulletsAtlas)
{
	...
	glm::vec4 textureCoords = bulletsAtlas.get(0, 0);

	if (isEnemy)
	{
		textureCoords = bulletsAtlas.get(1, 0);
	}
	...
	for (int i = 0; i < 5; i++)
	{
		...
		renderer.renderRectangle({ position - glm::vec2(50,50) + (float)i * 25.f * fireDirection, bulletSize,bulletSize },
